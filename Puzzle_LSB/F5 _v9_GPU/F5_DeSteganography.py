...
'''
@cuda.jit
def extract_message_bits_gpu(permuted_pixels, extracted_bits):
    idx = cuda.grid(1)
    if idx < extracted_bits.size:
        extracted_bits[idx] = permuted_pixels[idx] % 2
...
def f5_extract_message(key, stego_encoded_pixels, DeStego_Time_file):
    ...
    d_extracted_bits = cuda.device_array(len(permuted_pixels), dtype=np.uint8)

    threads_per_block = 256
    blocks_per_grid = (len(permuted_pixels) + (threads_per_block - 1)) // threads_per_block
    
    start_time = time.time()
    extract_message_bits_gpu[blocks_per_grid, threads_per_block](d_permuted_pixels, d_extracted_bits)
    extracted_bits = d_extracted_bits.copy_to_host()
    end_time = time.time()
...
def extract_actual_data_bits(extracted_bits, data_size, DeStego_Time_file):
    """Extract the actual data bits based on the data size."""
    start_time = time.time()
    actual_bits = extracted_bits[:data_size]
    end_time = time.time()
    execution_time = end_time - start_time
...
def F5_DeSteganography(in_image_path, out_filename, key, data_bit_count, DeStego_Time_file):        
    ...
    data_size, execution_time2 = compute_data_size(extracted_bits_red_channel[:data_bit_count], DeStego_Time_file)
    
    if data_size > (max_capacity_per_channel - data_bit_count):
        extracted_bits_green_channel, execution_time3 = f5_extract_message(key, green_channel, DeStego_Time_file)
    else:
        extracted_bits_green_channel = []
        execution_time3 = 0
    
    if data_size > (2 * max_capacity_per_channel - data_bit_count):
        extracted_bits_blue_channel, execution_time4 = f5_extract_message(key, blue_channel, DeStego_Time_file)
    else:
        extracted_bits_blue_channel = []
        execution_time4 = 0        
    ...
    extracted_bits = np.concatenate(
        (extracted_bits_red_channel[data_bit_count:], extracted_bits_green_channel, extracted_bits_blue_channel)
    )
    end_time_total = time.time()
    execution_time5 = end_time_total - start_time_total
    
    extracted_bits, execution_time6 = extract_actual_data_bits(extracted_bits, data_size, DeStego_Time_file)
    
    execution_time7 = save_bits_to_bin_file(extracted_bits, out_filename, DeStego_Time_file)
...
#     DeStego_Time_file = "V1_DeSteganography_time_complexity.txt"
#     F5_DeSteganography(in_image_path, out_filename, key, data_bit_count, DeStego_Time_file)
    
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Extract data from steganography image")
    ...
    parser.add_argument("--in_image_path", type=str, required=True, help="Path to the input steganography image file")
    parser.add_argument("--out_filename", type=str, required=True, help="Path to the output binary file for extracted data")
    parser.add_argument("--DeStego_Time_file", type=str, required=True, help="Path to the log file for de-steganography time complexities")

    args = parser.parse_args()

    F5_DeSteganography(args.in_image_path, args.out_filename, args.key, args.data_bit_count, args.DeStego_Time_file)

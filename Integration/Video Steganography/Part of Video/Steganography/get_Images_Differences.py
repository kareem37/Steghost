...
def process_folders(Stego_Images_folder, Original_Images_folder, output_file_path):
    input_files = sorted(os.listdir(Stego_Images_folder))
    user_files = sorted(os.listdir(Original_Images_folder))
    ...
    for input_file, user_file in zip(input_files, user_files):
        input_path = os.path.join(Stego_Images_folder, input_file)
        user_path = os.path.join(Original_Images_folder, user_file)

        if os.path.isfile(input_path) and os.path.isfile(user_path):
            diff_bits, bits, diff_percentage = calculate_bit_difference(input_path, user_path)
            ssim_score = calculate_ssim(input_path, user_path)
...
Original_Images_folder = 'User_Video_Frames'
output_file_path = 'User_Output_Statistics\Images_Differences.txt.txt'
